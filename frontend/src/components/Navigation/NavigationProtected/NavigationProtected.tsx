...
const NavigationProtected = () => {
  ...
  const navigationMenu = useMemo(() => {
    ...
    return navigation.filter((nav) => permissions.includes(nav.role));
  }, [session?.user]);

  return (
    <nav className={styles.navigation}>
      <section className={styles.section}>
        <span className={styles.statement}>{greetings}</span>
        <span className={styles.statement}>{nameOfUser}</span>
        <hr />
      </section>
