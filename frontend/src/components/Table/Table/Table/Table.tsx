...
function Table<D extends object>({ columns }: Props<D>): React.JSX.Element {
  ...
  const { getFooterGroups, getHeaderGroups, getRowModel } = table;

  useEffect(() => {
    setTable(table);
    setRowSelectionProvider(rowSelection);
  }, [table, rowSelection, setTable, setRowSelectionProvider]);
            ...
            {headerGroup.headers.map((header) => (
              <th key={header.id} className={styles['head-cell']}>
                {header.isPlaceholder ? null : (
                  <>
                    <div
                      className={header.column.getCanFilter() ? styles.sorting : ''}
